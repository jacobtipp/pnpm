...
module.exports = async (npmrc, {npmPublish, pkgRoot, tarballDir, tarball}, pkg, context) => {
  const {
    cwd,
    env,
    stdout,
  ...
  if (npmPublish !== false && pkg.private !== true) {
    ...
    const result = execa(
      'pnpm',
      ['publish', tarballDir ? `${path.resolve(cwd, tarballDir)}/${tarball}` : basePath , '--userconfig', npmrc, '--tag', distTag, '--registry', registry],
