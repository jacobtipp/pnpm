...
module.exports = async (npmrc, {tarballDir, pkgRoot}, {cwd, env, stdout, stderr, nextRelease: {version}, logger}) => {
  ...
  if (tarballDir) {
    ...
    const packResult = execa('pnpm', ['pack', '--dir', basePath], {cwd, env, preferLocal: true});
    packResult.stdout.pipe(stdout, {end: false});
    packResult.stderr.pipe(stderr, {end: false});

    const tarballArray = (await packResult).stdout.split('\n');
    logger.log('Tarball Array: $s', tarballArray)
    const tarball = tarballArray.pop()
